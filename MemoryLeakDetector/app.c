...
main(int argc, char **argv){
    ...
    student_t *namit = xcalloc(object_db, "student_t", 1);
    mld_set_dynamic_object_as_root(object_db, namit);

    student_t *samyak = xcalloc(object_db, "student_t", 1);
    strncpy(samyak->stud_name, "samyak", strlen("samyak"));
    //namit->best_colleage = samyak;

    emp_t *nagaraja = xcalloc(object_db, "emp_t", 2);
    mld_set_dynamic_object_as_root(object_db, nagaraja);
    nagaraja->p = xcalloc(object_db, "int", 1);
